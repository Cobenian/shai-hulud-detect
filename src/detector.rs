...
impl ShaiHuludDetector {
    ...
    pub fn scan(&mut self, directory: &Path) -> Result<ScanFindings> {
        ...
        self.scanner
            .check_workflow_files(directory, &mut findings)?;
        self.scanner.check_file_hashes(directory, &mut findings)?;
        self.scanner.check_packages(directory, &mut findings)?;
        self.scanner
            .check_postinstall_hooks(directory, &mut findings)?;
        self.scanner.check_content(directory, &mut findings)?;
        self.scanner
            .check_trufflehog_activity(directory, &mut findings)?;
        self.scanner.check_git_branches(directory, &mut findings)?;
        self.scanner
            .check_shai_hulud_repos(directory, &mut findings)?;
        self.scanner
            .check_package_integrity(directory, &mut findings)?;
        ...
        if self.config.paranoid_mode {
            ...
            self.scanner
                .check_network_exfiltration(directory, &mut findings)?;
        }
